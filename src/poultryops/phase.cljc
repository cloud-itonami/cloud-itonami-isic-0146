(ns poultryops.phase
  "Phase gate for the poultry-farm operations coordinator rollout (0->3
  maturity). Mirrors `swineops.phase` (cloud-itonami-isic-0145) in
  ...
  Phase 1: supervised operation -- always-escalate ops
    (`poultryops.governor/always-escalate-ops`) escalate even when clean;
    other ops commit if the Governor approves.
  ...
  (:require [poultryops.governor :as governor]))
